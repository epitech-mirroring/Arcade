        ...
        stage('🏗️ Build') {
            steps {
                ansiColor('xterm') {
                    ...
                    sh 'docker run --rm --security-opt "label:disable" -v "$(pwd)":"/mnt/delivery" -w "/mnt/delivery" epitechcontent/epitest-docker:latest git config --global --add safe.directory "/mnt/delivery" && ssh-keygen -t rsa -b 4096 -C && make'

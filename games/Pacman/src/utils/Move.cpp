...
void checkTeleport(GridCoordinate &grid) {
    ...
    } else if (grid.getX() >= MAP_WIDTH * 8.f * SCALE) {
        grid.setX(0);
    }
}
...
bool Move::isLegal(const Wall (&map)[37][28]) const {
    ...
    if (IS_GIZMOS(*arcade)) {
        GridCoordinate gridScreen = GridCoordinate(grid).toScreen();
        GridCoordinate frontScreen = GridCoordinate(front).toScreen();
        GIZMOS(*arcade)->drawRect(gridScreen, gridScreen + Coord2D(8 * SCALE, 8 * SCALE), false, isWalkable ? RGBAColor::GREEN : RGBAColor::RED);
        GIZMOS(*arcade)->drawRect(frontScreen, frontScreen + Coord2D(8 * SCALE, 8 * SCALE), false, isFrontWalkable ? RGBAColor::GREEN : RGBAColor::RED);
    }
